...
void sortQueue(queue &q) {
    ...
    while (!q.isEmpty()) {
        ...
        // Step 1: Find minimum element in current queue
        while (!q.isEmpty()) {
            int val = q.front();
            q.dequeue();
            temp.enqueue(val);
        ...
        // Step 2: Move all elements except minVal back to original queue
        bool minRemoved = false;
        while (size--) {
            int val = temp.front();
            temp.dequeue();
            if (val == minVal && !minRemoved) {
                minRemoved = true; // skip this min value
                continue;
            }
            q.enqueue(val);
        }

        // Step 3: Enqueue minVal to sortedQ
        sortedQ.enqueue(minVal);
    }

    // Step 4: Move sorted elements back to original queue
    while (!sortedQ.isEmpty()) {
        int val = sortedQ.front();
        sortedQ.dequeue();
        q.enqueue(val);
    }
}
